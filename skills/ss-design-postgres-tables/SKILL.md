...
description: >-
  ...
  or update-heavy tables, or understanding PostgreSQL-specific gotchas. Triggers on PostgreSQL schema, table design,
  PRIMARY KEY, FOREIGN KEY, indexes, B-tree, GIN, JSONB, constraints, normalization, partitioning, row-level security.
allowed-tools:
  - Bash
  - Read
  - Edit
...
## When to Use
- Designing new PostgreSQL tables, schemas, or data models
- Choosing data types, constraints, or indexes
- Creating reference tables (users, orders), JSONB schemas, or time-series tables
- Designing update-heavy, upsert-heavy, or insert-heavy workloads
- Understanding PostgreSQL best practices (normalization, partitioning, RLS)

## When NOT to Use
- Non-PostgreSQL databases — patterns here are PostgreSQL-specific
- Application logic or ORM-level design — this covers the SQL/schema layer only
- Query optimization or EXPLAIN analysis — this is about table structure, not query tuning
