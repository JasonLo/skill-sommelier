...
## Phase 2.5: Content Outline

Before generating the full presentation, write a Markdown outline to a file so the user can review and edit the content structure.

### Step 2.5.1: Generate the outline file

Based on content gathered in Phase 1 (and extracted content from Phase 4 if converting a PPT), write a file named `presentation-outline.md` in the current working directory.
...
<!-- Edit slide titles, bullets, and notes below. Add or remove slides as needed. -->
...
- 3–6 bullet points per slide (respecting content density limits)
- If content is too dense for one slide, split it into two slides now
- Include a brief speaker note (`>` blockquote) only when useful
- Don't over-generate — keep it editable at a glance

### Step 2.5.2: Ask the user to review

Tell the user:

```
I've written the slide outline to `presentation-outline.md`.

Open it, edit the content as needed — reorder slides, trim bullets, add anything missing — then come back and I'll generate the full presentation.

Ready when you are.
```

Then **wait** for the user to confirm they're done editing. Do not proceed to Phase 3 until confirmed.

### Step 2.5.3: Read the final outline

Once the user confirms, re-read `presentation-outline.md` from disk (not from memory) and use its current contents as the authoritative content for Phase 3.
